...
def log_video(frames: List, durations: List, curr_episode: int, episode_reward: int, fps: int):
    clip = ImageSequenceClip(frames, durations=durations)
    clip.write_videofile(f"train_video/mario_episode{curr_episode}_reward{episode_reward}.mp4",
                            fps=fps,
                            )
    wandb.log({f"gameplay": wandb.Video(f"train_video/mario_episode{curr_episode}_reward{episode_reward}.mp4",
